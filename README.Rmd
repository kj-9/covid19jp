...
```{r echo=FALSE}
covid19jp::pref %>%
  create_informant(tbl_name = "covid19jp::pref") %>%
  info_tabular(" " = "Master dataset for Japanese prefectures.",
               `Source` = "Statistics Bureau of Japan ([xlsx file](https://www.stat.go.jp/data/nihon/zuhyou/n200200200.xlsx)).") %>%
  info_columns(columns = "prefCode"   , `((en))` = "Prefecture code") %>%
  info_columns(columns = "prefCode"   , `((jp))` = "都道府県番号") %>%
  info_columns(columns = "prefJP"     , `((en))` = "Prefecture name in Japanese") %>%
  info_columns(columns = "prefJP"     , `((jp))` = "都道府県名(日本語)") %>%
  info_columns(columns = "prefEN"     , `((en))` = "Prefecture name in English") %>%
  info_columns(columns = "prefEN"     , `((jp))` = "都道府県名(英語)") %>%
  info_columns(columns = "population" , `((en))` = "Population estimate in thousands in 2018") %>%
  info_columns(columns = "population" , `((jp))` = "2018 年推計人口(千人)") %>%
  incorporate()
```
